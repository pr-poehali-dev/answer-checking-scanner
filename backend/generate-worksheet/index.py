...
def generate_worksheet_content(subject: str, class_num: int, topic: str,
    ...
    system = (
        ...
        "соответствует возрасту и классу.\n"
        "Рабочий лист построен по принципу ПРАКТИЧЕСКОЙ РАБОТЫ: ученик исследует материал, "
        "анализирует данные, рассуждает и сам приходит к ответу, а в конце формулирует вывод.\n"
        "ГЛАВНОЕ ПРАВИЛО ПРИЛОЖЕНИЙ: приложенные данные (таблица, текст-источник, карта, "
        "изображение) дают ученику пищу для размышления, но НЕ содержат ни прямого, ни косвенного "
        "ответа на вопрос задания. Ответ не должен быть написан в приложении или легко списан из "
        "него — ученик обязан подумать, сопоставить, вычислить или сделать вывод сам. При этом по "
        "приложенным данным должно быть ПОНЯТНО и реально возможно догадаться до ответа.\n"
    ...
    img_rule = (
        ...
        '"портрет Пушкина"). Изображение связано с вопросом, но само по себе не даёт готового ответа. '
    ...
    user = (
        ...
        f"Составь практический рабочий лист по теме строго по программе Минпросвещения РФ для {class_num} класса.\n"
        f"Нужно РОВНО {tasks_count} заданий разного типа (анализ таблицы данных, работа с "
        f"текстом-источником, работа с картой/изображением, задача с рассуждением, сравнение и "
        f"классификация, объяснение причин и следствий и т.п.).\n"
        "Верни JSON строго в формате:\n"
        '{\n'
        '  "title": "Краткое название рабочего листа",\n'
        '  "intro": "1-2 предложения: цель практической работы и что ученик научится делать",\n'
        '  "tasks": [\n'
        '    {"number": 1, "type": "Тип задания", "instruction": "Развёрнутая формулировка задания для ученика", '
        '"content": "Текст-источник или данные для анализа, БЕЗ готового ответа; иначе пустая строка", '
        ...
        '  ],\n'
        '  "conclusion": "Текст для итогового блока: 1-2 вопроса или фраза-зачин, по которым ученик сам формулирует вывод по работе"\n'
        '}\n'
        "Требования:\n"
        f"- РОВНО {tasks_count} заданий в массиве tasks\n"
        "- Задания НЕ односложные: формулируй развёрнуто, проси объяснить, сравнить, доказать, "
        "сделать вывод, привести 2-3 примера, рассуждать. Минимум одно действие мышления в каждом задании.\n"
        "- ВАЖНО: приложение даёт данные для анализа, но НЕ содержит прямого или косвенного ответа. "
        "Нельзя списать ответ из таблицы/текста — нужно подумать и вывести его самому. "
        "Но по данным должно быть понятно и возможно догадаться до правильного ответа.\n"
        "- Для заданий на анализ данных используй поле \"table\" с заголовками и строками "
        "(реальные осмысленные данные по теме, без столбца «ответ»). Если таблица не нужна — не добавляй поле \"table\".\n"
...
def build_docx(content: dict, subject: str, class_num: int, topic: str,
    ...
    concl_head.paragraph_format.space_before = Pt(14)
    concl_head.paragraph_format.space_after = Pt(2)
    rch = concl_head.add_run("Вывод")
    rch.bold = True
    rch.font.size = Pt(13)
    rch.font.color.rgb = RGBColor.from_string(BRAND_ACCENT)

    conclusion = content.get("conclusion")
    if conclusion:
        cp = doc.add_paragraph()
        cp.paragraph_format.space_after = Pt(3)
        rcp = cp.add_run(conclusion)
        rcp.italic = True
        rcp.font.size = Pt(11)
