...
def gigachat_chat(messages: list, max_tokens: int = 1500, temperature: float = 0.4,
                  model: str = "GigaChat", req_timeout: int = 55, max_retries: int = 3) -> str:
...
def generate_outline(topic: str, description: str, slides_count: int, audience: str) -> dict:
    ...
    system = (
        "Ты учитель-методист. Создай структуру учебной презентации в JSON. "
        "Пиши конкретно: факты, даты, цифры, термины, имена. Без общих фраз. "
        "Заголовки слайдов: 4–6 слов. Тезисы: 10–18 слов, 4–5 штук на слайд. "
        "fact: один интересный факт по теме слайда (1 предложение)."
    )
    user = (
        f"Тема: {topic}\n"
        ...
        "Ответ — ТОЛЬКО JSON:\n"
        '{"subtitle":"подзаголовок 6-10 слов","slides":['
        '{"title":"...","bullets":["...","...","...","...","..."],"fact":"..."}],'
        '"conclusion":["итог1","итог2","итог3","итог4"]}\n'
        f"slides — ровно {slides_count} элементов."
    )

    # 170 токенов на слайд — достаточно для конкретного контента
    max_tok = min(170 * slides_count + 400, 2200)
