...
def _gigachat_vision(image_b64: str, questions_count: int) -> dict:
    ...
    # Промпт адаптирован под бланк examica-стиля: кружки A/B/C/D
    prompt = (
        f"Это бланк ответов с сеткой. Каждая строка — один вопрос (1–{questions_count}). "
        f"В строке несколько кружков с буквами (A, B, C, D или другие). "
        f"Закрашенный / зачернённый кружок = выбранный ответ. "
        f"Верни ТОЛЬКО JSON без пояснений:\n"
        f'{{"code":"XXXXX","answers":["A","C","B",...]}}\n'
        f"code = 5 цифр из поля «КОД УЧЕНИКА» (? если неразборчиво).\n"
        f"answers = ровно {questions_count} элементов: заглавная буква выбранного варианта, "
        f"пустая строка если кружок не закрашен. Только JSON."
    )
